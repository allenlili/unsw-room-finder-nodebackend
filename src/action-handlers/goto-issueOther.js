...
/*
 * @module service/dispatcher/handlers/goto-issueOther
 */
...
/**
 * Sends a response to the user, asking them what the issue is.
...
export default async function (context: Context, action: Action): Promise<void> {
    ...
    // tell the state the follow is suppose to be issueOther
    const stateUpdate = { data: { ...state.data, followUp: 'goto/issueOther/followUp' } };
...
/**
 * Sends a response to the user, capturing their issue and thanking them for their feedback.
 * @param {Context} context application context
 * @param {Action} action user action
 * @returns {Promise<void>} promise eventuating to void
 */
export async function followUp (context: Context, action: Action): Promise<void> {
    if (action.type !== 'goto/issueOther/followUp') throw new TypeError();
